...
```{r}
library(readxl)
files <- read_xlsx("file.inventory_combined2.xlsx")
...
plot <- ggplot(data = sub, aes(PHATE1, PHATE2)) +
  geom_point(stroke = 0, size = 0.5, color = "grey") +
    facet_grid(.~tissue)

plot + stat_density_2d(subset(sub, mtD2_pos == "Yes"), mapping = aes(PHATE1, PHATE2, fill = ..level..), geom = "polygon")  + 
    scale_fill_viridis() + 
    theme_void() + 
    guides(fill = F) + 
    theme(strip.background = element_blank(),
      strip.text = element_blank())
ggsave("Overall_mtD2posDensity_byPhate.png", height = 4, width = 12.75, dpi = 600)
...
pal <- mycolors[unique(sub$cluster.sq)]
ggplot(data = set1, aes(PHATE1, PHATE2, color = as.factor(cluster.sq))) +
  geom_point(stroke = 0) +
    scale_color_manual(values = pal) + 
    theme_void() +
    theme(strip.background = element_blank(),
...
ggplot(data = set1, aes(PHATE1, PHATE2, color = curve1)) +
  geom_point(stroke = 0) +
    scale_color_viridis() + 
    theme_void() +
    theme(strip.background = element_blank(),
...
ggplot(data = set1, aes(PHATE1, PHATE2, color = curve2)) +
  geom_point(stroke = 0) +
    scale_color_viridis() + 
    theme_void() +
    theme(strip.background = element_blank(),
...
ggplot(set2, aes(x=curve2, y =mtD2)) + 
  geom_point(aes(color = as.factor(cluster.sq)), size = 0.5) + 
  geom_smooth(method = "loess", se=FALSE, color = "black") + 
  facet_grid(chow~tissue) + 
  geom_hline(yintercept = 0, lty = 2) + 
  scale_color_manual(values = pal) + 
   guides(color = F) + 
    theme_void() +
...
ggsave("Set2_mtD2curve2_PT.png", height = 6, width = 6, dpi = 600)
...
ggsave("Overall_mtD2curve2_PT.png", height = 2, width = 6, dpi = 600)
```
