...
```{r}
library(readxl)
files <- read_xlsx("file.inventory.xlsx")
```
...
```{r}
annot <- readxl::read_xlsx("cluster.annotations.xlsx")
...
annot <- readxl::read_xlsx("cluster.annotations.xlsx")[,1:4]
...
```{r}
annot <- readxl::read_xlsx("adipose.clusters.xlsx")
...
x <- as.data.frame(colData(tmp)) %>%
    mutate(pos = ifelse(mtD2 >= 2, "yes", "no"))
