...
mycolors <- RColorBrewer::colorRampPalette(brewer.pal(8, "Set1"))(length(unique(tmp$cluster.sq)))
...
############################################
#Getting Numbers of Cells from sampled data
############################################

clusters <- tmp$cluster.sq
tmp$cellType <- NA
for(i in seq_along(clusters)) {
  tmp$cellType[i] <- names(list)[grep(paste0("\\b", clusters[i], "\\b") , list)]
}

summary <- tmp %>%
 dplyr::group_by(sample_id, cellType) %>%
  count()

summary <- summary %>%
  dplyr::group_by(sample_id) %>%
  mutate(sum.sample = sum(n))

############################################
#Getting Numbers of Cells from total data
############################################
sce <- readRDS("sce_flowset_filtered_logical.rds")
meta <- as.data.frame(sce@colData)


total.sumamry <- as.data.frame(table(meta$sample_id))
colnames(total.sumamry)[2] <- "total.cells"
meta <- meta[,c(1,3,4,5)]
meta <- unique(meta)
...
##################################################
#Combining the results of sampled and total cells
################################################

summary <- merge(summary, table, by = "sample_id")
summary <- merge(summary, total.sumamry, by.x = "sample_id", by.y = "Var1")
summary <- merge(summary, meta, by = "sample_id")

#Calculating numbers per gram
summary$est.num.per.gram <- ((summary$n/summary$sum.sample)*summary$total.cells)/summary$adipose.processed

#Organizing data to graph

summary$set <- stringr::str_split(summary$sample_id, "_", simplify = TRUE)[,1]
summary$tissue <- stringr::str_split(summary$sample_id, "_", simplify = TRUE)[,2]
summary$tissue <- stringr::str_split(summary$tissue, "-", simplify = TRUE)[,1]
summary$tissue <- ifelse(summary$tissue == "iWat", "iWAT", summary$tissue)

summary$tissue <- factor(summary$tissue, levels = c("eWAT", "iWAT", "BAT"))
summary$cellType <- factor(summary$cellType, levels = c("Macs", "Mono", "Neutr", "DCs", "Eos", "ILC2", "NK", "Tcells", "Bcells", "NI"))


df2 <- data_summary(summary, varname="est.num.per.gram", 
                    groupnames=c("tissue", "cellType"))

ggplot(df2, aes(x = cellType, y = est.num.per.gram, fill = tissue)) + 
  geom_bar(stat = "identity", position = "dodge", color = "black") + 
  geom_errorbar(aes(ymin=est.num.per.gram-(sd/sqrt(27)), ymax=est.num.per.gram+(sd/sqrt(27))), width=.5,
                 position=position_dodge(.9)) + 
  scale_fill_manual(values = c("white", "#DFC7AE", "#9E5C25")) + 
  theme_classic() + 
  ylab("Cells/gram adipose") + 
  theme(axis.title.x = element_blank())


stat.test <- summary %>%
  group_by(cellType) %>%
  t_test(est.num.per.gram ~ tissue) 
```
...
EWAT <- tmp[tmp$tissue == "eWAT",]
IWAT <- tmp[tmp$tissue == "iWAT",]
BAT <- tmp[tmp$tissue == "BAT",]
tissue.list <- list(EWAT, IWAT, BAT)

plot1 <- ggplot(data = EWAT, aes(x=X1, y=X2)) +
  geom_point(stroke = 0, size = 0.5, color = "grey") 
plot1 <- plot1 + stat_density_2d(subset(EWAT, mtD2_pos == "Yes"), mapping = aes(x = X1, y = X2, fill = ..level..), geom = "polygon", h =2)  + 
    scale_fill_viridis() + 
    guides(fill = F) + 
    theme_void() + 
...
plot2 <- ggplot(data = IWAT, aes(x=X1, y=X2)) +
  geom_point(stroke = 0, size = 0.5, color = "grey") 
plot2 <- plot2 + stat_density_2d(subset(IWAT, mtD2_pos == "Yes"), mapping = aes(x = X1, y = X2, fill = ..level..), geom = "polygon", h =2)  + 
    scale_fill_viridis() + 
    guides(fill = F) + 
    theme_void() + 
    theme(strip.background = element_blank(),
...
plot3 <- ggplot(data = BAT, aes(x=X1, y=X2)) +
  geom_point(stroke = 0, size = 0.5, color = "grey") 
plot3 <- plot3 + stat_density_2d(subset(BAT, mtD2_pos == "Yes"), mapping = aes(x = X1, y = X2, fill = ..level..), geom = "polygon", h =2)  + 
    scale_fill_viridis() + 
    guides(fill = F) + 
    theme_void() + 
    theme(strip.background = element_blank(),
