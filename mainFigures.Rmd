...
plot1 + plot2 + plot3
ggsave("./outputs/Overall_mtD2_positive.png", height = 4, width = 12.75, dpi = 600)
```
...
clusters2Type <- unique(sub[,c("cluster.sq", "Major")])
...
y <- merge(y, clusters2Type, by = "cluster.sq")

update_geom_defaults("point", list(alpha=1))

#Multiple levels of annotation here - 
#geom_bar will be the relative percent of mtD2+ cells in the clusters
#geom_point will be the size of the cluster
#geom_rect will be the cell type labeling for the cluster - this requires looping through each cluster 
#and adding a rectangle manually - it is not pretty, but it gets the job done and create a ggproto object
#with 23 layers. 

labels <- NULL
pal <- tableau_color_pal()(10)
names(pal) <- sort(unique(clusters2Type$Major))
clusters <- unique(clusters2Type$cluster.sq)
major <- sort(unique(clusters2Type$Major))
for(i in seq_along(clusters)) {
  count <- as.numeric(i)
  if (i == 1) {
    labels <- ggplot(y, aes(x = as.factor(cluster.sq), y=scale*100)) +
      geom_bar(stat = "identity") +
      geom_point(aes(color = as.factor(cluster.sq), y = axis*70, size = sum)) + 
      geom_rect(mapping=aes_string(xmin=(count-0.5), xmax=(count+0.5), ymin=85, ymax=100), fill = unname(pal[clusters2Type[i,"Major"]])) 
      
      
      
  }
  else{
    labels <- labels + geom_rect(mapping=aes_string(xmin=(count-0.5), xmax=(count+0.5), ymin=85, ymax=100), fill = unname(pal[clusters2Type[i,"Major"]]))
  }
}
...
ggsave("./outputs/MtD2_proportion_set1.pdf", height = 4.5, width=8)
```
...
ggsave("./outputs/MtD2_proportion_set2.pdf", height = 6.75, width=8)
```
